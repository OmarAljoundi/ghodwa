...
export async function generateMetadata({
  ...
  const { seo, ar_name, en_name, logo } = await getBrandBySlug(brandSlug, 'security_defence');
  ...
  const dictionary = generatePageBilingualSeo(
    (seo as SeoSchema) ?? {},
    `/security-and-defence/${brandSlug}`,
    images,
  )[locale];
...
export default async function Page({ params }: { params: Promise<{ brandSlug: string }> }) {
  ...
  return (
    <BrandDetails
      dataPromise={getBrandBySlug(brandSlug, 'security_defence')}
      basePath="/security-and-defence"
    />
  );
}
