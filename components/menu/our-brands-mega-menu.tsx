...
export default function OurBrandsMegaMenu({ brands }: { brands: BrandWithRelationsSchema[] }) {
      ...
      <div className={'grid grid-cols-4 gap-6'}>
        {brandsFilter.map(({ name, categories, slug }) => (
          <div key={name}>
            ...
            <ul>
              {categories?.map((o) => (
                <CategoryItem brandSlug={slug} category={o as Category} key={o.id} />
              ))}
            </ul>
          </div>
        ))}
...
function CategoryItem({ category, brandSlug }: { category: Category; brandSlug: string }) {
