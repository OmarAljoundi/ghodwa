...
export function NewsContainer({
  ...
  dataPromise: ReturnType<typeof getNews>;
}) {
...
const NewsItem = ({ newsItem }: { newsItem: News }) => {
  const { createdAt, summary, title, slug, image } =
    useFilteredLanguageData(newsItem);
  ...
  return (
    <Link href={`/news/${slug}`}>
      ...
      >
        <div
          className={cn(
            "flex flex-col md:flex-row md:justify-start md:gap-x-4 gap-y-3"
          )}
        >
          <div
            className={cn(
          ...
          <div className="flex flex-col gap-y-3 min-w-0 flex-1">
            <div className="flex gap-x-2 justify-start font-light text-xl items-center min-w-0">
              <h1 className="truncate transition-colors text-gray-800">
                {title}
              </h1>
