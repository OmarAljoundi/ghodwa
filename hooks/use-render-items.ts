...
export function useExtraPages(settings: SettingSchema, type?: 'menu' | 'footer') {
...
export function useServicesPages(services: Service[], type?: 'menu' | 'footer') {
  const allAboutPages = useMemo(() => {
...
export function useBrandsPages(services: BrandWithRelationsSchema[], type?: 'menu' | 'footer') {
  const allAboutPages = useMemo(() => {
    const mappedExtraPages = services.filter((x) => {
      if (type === 'menu') return x.showOnMenu;
